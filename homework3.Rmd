...
```{r ans11}
finStatement["賠錢1"] <- c(finStatement[["營業利益（損失）"]]<0)
loss <- finStatement[finStatement$賠錢==T,]
loss1Companies <- c(loss[["公司名稱"]])
loss1Companies
```
...
```{r ans12}
finStatement[["賠錢2"]] <- c(finStatement$`基本每股盈餘（元）`<0)
loss2 <- finStatement[finStatement$賠錢2==T,]
loss2Companies <- c(loss2[["公司名稱"]])
loss2Companies
```
...
```{r ans13}
factor1 <- c(finStatement[["賠錢1"]]==F&finStatement[["賠錢2"]]==T)
factor2 <- c(finStatement[["賠錢1"]]==T&finStatement[["賠錢2"]]==F)
finStatement["單一條件"] <- c(factor1==T|factor2==T)
only1 <- finStatement[finStatement$單一條件==T,]
only1LossCompanies <- c(only1[["公司名稱"]])
only1LossCompanies
```
...
```{r ans23}
dataframe_show_info <- function(exhibitionInput){
df_oneShow <- data.frame(title=exhibitionInput[["title"]],
...
```{r ans24}
change <- as.factor(df_allShows$sourceWebName)
df_allShows$sourceWebName <- change
